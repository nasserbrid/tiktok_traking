...
from tracking.services.tiktok_live_service import check_if_live
import logging
...
def check_all_accounts(self):
    """
    Vérifie tous les comptes TikTok pour détecter les lives.
    S'exécute automatiquement toutes les X minutes via Celery Beat.
    """
    comptes = CompteTiktok.objects.all()
    ...
    for compte in comptes:
        try:
            ...
            error_count += 1
            logger.exception(f"❌ Erreur pour @{compte.username}: {str(e)}")
...
def check_single_account(compte_id):
    ...
    try:
        ...
        logger.warning(f"❌ Compte introuvable avec ID {compte_id}")
        return "❌ Compte introuvable"
    except Exception as e:
        logger.exception(f"❌ Erreur lors de la vérification du compte {compte_id}: {e}")
...
def send_notification_async(live_id):
    ...
    try:
        ...
        # Envoi la notification via la fonction utils
        envoyer_notification_live(live)
        logger.info(f"✅ Notification envoyée pour @{live.compte.username}")
        return f"✅ Notification envoyée pour {live.compte.username}"
    except Live.DoesNotExist:
        logger.warning(f"❌ Live introuvable avec ID {live_id}")
        return "❌ Live introuvable"
    except Exception as e:
        logger.exception(f"❌ Erreur lors de l'envoi notification live {live_id}: {e}")
        return f"❌ Erreur: {str(e)}"
